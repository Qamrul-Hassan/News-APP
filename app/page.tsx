// app/page.tsx
// Server Component: Fetch news on server-side using NewsAPI directly in the page

import Image from 'next/image';
import { NewsArticle } from './types';  // adjust path to your types

// Hardcoded API key for testing only. Remove before production!
...
export default async function Home() {
  ...
  const res = await fetch(
    `${BASE_URL}?category=general&apiKey=${API_KEY}`,
    ...
    <main className="min-h-screen py-12 px-6 bg-gradient-to-b from-white to-gray-100">
      <header className="text-center mb-12">
        ...
        <p className="text-gray-600 text-lg">
          Your daily dose of the latest news
        </p>
      </header>
